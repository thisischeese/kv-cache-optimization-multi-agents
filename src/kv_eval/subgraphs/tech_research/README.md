...
청크는 자르지 않고 통째로 LLM에 보여 줍니다(`MAX_PASSAGE_CHARS=4000`). v2 분할기는 표나 수식을 앞 문장에 붙여 두기 때문에 2200자보다 긴 청크가 생깁니다(두 원 논문에서 최대 3162자). 이전 한도(2600자)에서는 InfiniGen p.7 동작 원리 설명의 뒷부분이 잘렸습니다. 두 원 논문의 모든 청크가 한도 안에 드는지는 테스트로 확인합니다.
...
- v2 컬렉션(`kv_cache_docs_v2`)으로 다시 돌렸을 때도 14개 항목이 모두 채워졌고 인용 70건이 자동 검사를 통과했습니다. 표 값을 다른 모델에 붙인 문장 2건은 표 검사로 버려졌습니다.
...
1. **임베딩 호출 잠금 (급함):** `embed_query()`에 잠금이 없습니다. 메인 그래프에서 TRL, 이해관계자, 도메인 Agent가 같은 단계에서 동시에 `retrieve()`를 불러, Apple MPS에서 `app.py`가 매번 죽습니다(exit 134, v2에서 3회 모두). 임베딩 호출을 `threading.Lock`으로 감싸 주시면 모든 Agent에 한 번에 적용됩니다. 그 전까지는 `.env`의 `EMBEDDING_DEVICE=cpu`로 피할 수 있습니다.
2. **분수 수식:** KIVI p.6의 "window size is expected to be R/2"가 "R 2"로 들어가 있어, LLM이 R^2로 잘못 옮긴 적이 있습니다. v2에서도 "R\n\n2"로 같습니다. 한계로 기록만 해도 됩니다.
3. **표의 모델 이름 위치:** KIVI p.8 표 3에서 병합 셀의 모델 이름이 그 모델의 행들 뒤에 추출됩니다(PDF 좌표로 확인). v2에서도 같아, 이 모듈은 표 안에만 있는 숫자를 쓴 문장을 버립니다.
4. `chunk_id` 규칙(`chunk_id_for_chunk`)이나 컬렉션 이름이 바뀌면 오프라인 검색기의 ID도 따라 바뀝니다. 이 모듈이 그 함수를 직접 부르기 때문입니다.
